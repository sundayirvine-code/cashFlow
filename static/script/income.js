...
// Function to populate the income transactions table
function populateIncomeTransactionsTable() {
    // Make an AJAX request to fetch income transactions
    fetch('/get_income_transactions')
        .then((response) => {
            if (!response.ok) {
        ...
        .then((data) => {
            ...
            data.income_transactions.forEach((transaction) => {
                const row = document.createElement('tr');
                const amount = formatIncome(transaction.amount)
                row.innerHTML = `
                <td>${transaction.transaction_id}</td>
                <td>${transaction.income_category_name}</td>
                <td>${transaction.description}</td>
                <td>${transaction.date}</td>
                <td class="income_amount_cell">${amount}</td>
                <td class="actions">
                    <i class="fas fa-edit edit-transaction" data-transaction-id="${transaction.transaction_id}" data-description="${transaction.description}" data-amount="${transaction.amount}" data-date="${transaction.date}"></i>
                    <i class="fas fa-trash-alt delete-transaction" data-transaction-id="${transaction.transaction_id}"></i> 
                </td>
            `;
            ...
            document.getElementById('fromincomeSummaryDateRange').textContent = formattedStartDate;
            document.getElementById('toincomeSummaryDateRange').textContent = formattedEndDate;
...
window.addEventListener('load', populateIncomeTransactionsTable);
